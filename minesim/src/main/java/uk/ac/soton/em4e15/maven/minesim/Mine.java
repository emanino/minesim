...
public class Mine {
	...
	private void createLayout() {
		...
		for(int i = 0; i < nSectionsMainTunnel; ++i) {
			...
			new MainTunnel(state, head, tail, nAtoms, new LayoutAtomStatus(prop, -1), atomRadius, layoutAtomtoUpdate,false);
			...
			if(i < nSectionsEscapeTunnel) {
				...
				new EscapeTunnel(state, head_bis, tail_bis, nAtoms, new LayoutAtomStatus(prop, -1), atomRadius, layoutAtomtoUpdate, false);
				if(leftSideJunctions.contains(i))
					new Tunnel(state, tail, tail_bis, nAtomsConnect, new LayoutAtomStatus(prop, -1), atomRadius, layoutAtomtoUpdate, true, false);
				else
					new EscapeTunnel(state, tail, tail_bis, nAtomsConnect, new LayoutAtomStatus(prop, -1), atomRadius, layoutAtomtoUpdate, false);
			}
	...
	private boolean extendTunnel(Position head, Direction dir, int nSections, int nAtoms, double atomRadius, int nDirChanges) {
		...
		new Tunnel(state, head, tail, nAtoms, las, atomRadius, layoutAtomtoUpdate, true,layoutRand.nextDouble() < Double.parseDouble(prop.getProperty("probGeofenced")));
